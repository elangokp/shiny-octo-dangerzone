...
public class GetSitesForCrawl implements Runnable {

	private int batchSize = 1;
	private BlockingQueue<ShopifySite> pendingSitesQueue;
	
	public GetSitesForCrawl(BlockingQueue<ShopifySite> pendingSitesQueue, int batchSize) {
	...
	public void run() {
		...
		while(isNextSiteAvailable) {
			try {
				...
				List<ShopifySite> sites = dbutils.getNextActiveSitesForCrawl(batchSize);
