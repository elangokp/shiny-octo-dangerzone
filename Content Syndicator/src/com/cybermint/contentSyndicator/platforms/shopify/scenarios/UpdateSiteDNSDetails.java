...
public class UpdateSiteDNSDetails implements Runnable {

	private BlockingQueue<ShopifySite> siteDetailsQueue;
	private int recordsPerBatch;
	
	public UpdateSiteDNSDetails(BlockingQueue<ShopifySite> siteDetailsQueue, int recordsPerBatch) {
		...
		while(sites.size() <= this.recordsPerBatch || ignoreBatchMinimum) {
			try {
				if(sites.size() >= this.recordsPerBatch) {
					...
					dbutils.updateSiteDNSDetails(sites);
					sites.clear();
