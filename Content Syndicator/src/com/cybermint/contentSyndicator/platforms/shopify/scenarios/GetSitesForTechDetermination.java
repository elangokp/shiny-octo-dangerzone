...
public class GetSitesForTechDetermination implements Runnable {

	private int batchSize = 1;
	private BlockingQueue<ShopifySite> pendingSitesQueue;
	
	public GetSitesForTechDetermination(BlockingQueue<ShopifySite> pendingSitesQueue, int batchSize) {
	...
	public void run() {
		...
		while(isNextSiteAvailable) {
			try {
				...
				List<ShopifySite> sites = dbutils.getNextSitesForTechDetermination(batchSize);
