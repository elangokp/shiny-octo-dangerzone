 ...
 */
class WP_Insights_Page_Versions_List_Table extends WPI_WP_List_Table {
    
    protected $assets_url = null;
    protected $view_url = null;
    protected $page_cache_id = null;
    
    /** ************************************************************************
     * REQUIRED. Set up a constructor that references the parent constructor. We 
     * use the parent reference to set some default configs.
     ***************************************************************************/
    function __construct($page_cache_id){
        global $status, $page;
        ...
        $this->page_cache_id = $page_cache_id;
    ...
    function admin_footer() {
    	...
    	echo '.wp-list-table .Versions .column-url { width: 50%; }';
    	echo '.wp-list-table .Versions .column-visits { width: 10%; }';
    	echo '.wp-list-table .Versions .column-browsing_time { width: 20%; }';
    	echo '.wp-list-table .Versions .column-actions { width: 20%; }';
    	echo '</style>';
    }
   ...
   /*  function column_default($item, $column_name){
    ...
    function column_version($item){
    	return sprintf('<a href="%1$s%2$s" target="_blank">Version %3$s</a>',
    			plugins_url("/../wpicache/"),
    			$item['file_name'],
    			$item['row_number']
    	);
    }
    
    function column_live_till($item){
    	return sprintf('%1$s',
    			$item['live_till']
    	);
    }
   ...
   /* function full_row_actions($item) {
    	...
    	$rowActions = '<div id="wpi-cache-id-'.$item['cache_id'].'-stats-details" class="wpi-cache-stats-details">';
    	$rowActions .= 
    	$rowActions .= '</div>';
    	return $rowActions;
    }*/
    ...
    function prepare_items() {
        ...
        */
        ...
        $sql = "SELECT cache.url, 
			        ...
			        cache.saved AS live_till,
			        cache.file AS file_name,
					...
					SELECT url
					FROM $cacheTable
					WHERE id = $this->page_cache_id
					)
					)
					GROUP BY records.cache_id
					ORDER BY records.cache_id ASC ";
